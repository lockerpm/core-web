...
import commonAccessServices from '../../../../../../services/common';
...
function ResetMasterPasswordFormData(props) {
  ...
  const handleConfirm = async () => {
    form.validateFields().then(async (values) => {
      ...
      const passwordStrength = await commonAccessServices.password_strength(values.new_password);
      const payload = {
      ...
      await emergencyAccessServices.password(item.id, payload)
        .then(() => {
          global.pushSuccess(t('notification.success.emergency_access.takeover_master_password', { user: item.email }))
          setCallingAPI(false);
          onClose();
        }).catch((error) => {
